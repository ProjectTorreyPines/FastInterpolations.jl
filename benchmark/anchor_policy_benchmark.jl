...
println("$(round(t_mono_linear * 1.0e6, digits = 2)) μs ($(round(t_mono_linear * 1.0e9 / 1000, digits = 2)) ns/query)")

print("Random queries:    ")
t_rand_linear = @belapsed FastInterpolations._fill_anchors!($buffer_linear, $x, $xq_random, Val(:linear))
println("$(round(t_rand_linear * 1.0e6, digits = 2)) μs ($(round(t_rand_linear * 1.0e9 / 1000, digits = 2)) ns/query)")
...
println("$(round(t_mono_constant * 1.0e6, digits = 2)) μs ($(round(t_mono_constant * 1.0e9 / 1000, digits = 2)) ns/query)")

print("Random queries:    ")
t_rand_constant = @belapsed FastInterpolations._fill_anchors!($buffer_constant, $x, $xq_random, Val(:constant))
println("$(round(t_rand_constant * 1.0e6, digits = 2)) μs ($(round(t_rand_constant * 1.0e9 / 1000, digits = 2)) ns/query)")
...
println("$(round(t_mono_quadratic * 1.0e6, digits = 2)) μs ($(round(t_mono_quadratic * 1.0e9 / 1000, digits = 2)) ns/query)")

print("Random queries:    ")
t_rand_quadratic = @belapsed FastInterpolations._fill_anchors!($buffer_quadratic, $x, $xq_random, Val(:quadratic))
println("$(round(t_rand_quadratic * 1.0e6, digits = 2)) μs ($(round(t_rand_quadratic * 1.0e9 / 1000, digits = 2)) ns/query)")
...
println("$(round(t_mono_cubic * 1.0e6, digits = 2)) μs ($(round(t_mono_cubic * 1.0e9 / 1000, digits = 2)) ns/query)")

print("Random queries:    ")
t_rand_cubic = @belapsed FastInterpolations._fill_anchors!($buffer_cubic, $x, $xq_random)
println("$(round(t_rand_cubic * 1.0e6, digits = 2)) μs ($(round(t_rand_cubic * 1.0e9 / 1000, digits = 2)) ns/query)")

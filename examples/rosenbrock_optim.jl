...
zvis = [log10(max(1e-6, rosenbrock(xi, yi))) for yi in yvis, xi in xvis]

# Panel 1: contour + trajectories
p1 = contourf(xvis, yvis, zvis; levels=30, color=:inferno,
...
p2 = plot(0:length(dist_fdm)-1, dist_fdm; alpha=0.6,lw=2.5, color=:cyan, marker=:circle, ms=6,
    yscale=:log10, label="FDM",
    xlabel="Iteration", ylabel="‖x - x*‖", title="Distance to True Minimum", legend=:bottomleft)
plot!(p2, 0:length(dist_ad)-1, dist_ad; alpha=0.6, lw=2.5, color=:orange, marker=:square, ms=6,
    label="AD (ForwardDiff)")
plot!(p2, 0:length(dist_ana)-1, dist_ana; alpha=0.6, lw=2.5, color=:lime, marker=:diamond, ms=6,
    label="Analytical")

plt = plot(p1, p2; layout=(1, 2), size=(1050, 420), margin=4Plots.mm, dpi=150,

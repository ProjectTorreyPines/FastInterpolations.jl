...
function validate_approximation()
    ...
    for degree in [3, 4, 5, 6]
        ...
        for ξ in test_ξs
            ...
            # Relative errors
            push!(w_errors, abs(w_approx - w_ref) / (abs(w_ref) + 1.0e-10))
            push!(wp_errors, abs(wp_approx - wp_ref) / (abs(wp_ref) + 1.0e-10))
            push!(wpp_errors, abs(wpp_approx - wpp_ref) / (abs(wpp_ref) + 1.0e-10))
        end

        @printf("\n  Degree %d Results:\n", degree)
        @printf(
            "    w:    max_error = %.2e (mean = %.2e)\n",
    ...
    return println("Use degree 5 polynomial for <1% error with fast evaluation")
end

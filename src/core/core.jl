...
# Order: types → ops → bc → grid → search → utils → nd_utils → series
...
include("nd_utils.jl")         # 8. ND-specific utilities (shared by constant/linear/cubic ND)
include("thomas_lu_solver.jl") # 9. Thomas algorithm (TDMA) solvers for tridiagonal systems
include("series_utils.jl")     # 10. Series validation helpers
include("series_matrix.jl")    # 11. Lazy transpose infrastructure

...
`_cache_axis(_convert_copy(x, T), bc)`. Cell widths read via `_get_h(cache.x, idx)`
...
- `Tz`: Element type of z coefficients (`= _promote_eltype(_coeff_op, Tg, Tv)` — Dual when grid is Dual)

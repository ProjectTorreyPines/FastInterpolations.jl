...
# ---- unified `interp` routing ------------------------------------------------
# The gridded evaluator is linear-only today, so the unified `interp`/`interp!`
# accept a GriddedQuery only when every axis method is `LinearInterp`, then
# forward to the linear gridded one-shot. bc travels from the per-axis methods;
# `coeffs`/`search`/`hint` are inert for linear (no coeff build; anchors search
# once per axis) and are accepted only for signature uniformity. These live here
# (not in hetero/) because `GriddedQuery` is defined after `hetero.jl` is
# included, while `interp` itself already exists by this point.
@inline _all_linear_methods(::Tuple{Vararg{LinearInterp}}) = true
@inline _all_linear_methods(::Tuple) = false
@inline _method_bc(m::LinearInterp) = m.bc
@noinline _throw_gridded_nonlinear(mt) = throw(
    ArgumentError("GriddedQuery evaluation currently supports only LinearInterp on every axis; got method = $mt")
)
...
function interp(
    ...
    _all_linear_methods(method_tuple) || _throw_gridded_nonlinear(method_tuple)
    return linear_interp(grids, data, gq; bc = map(_method_bc, method_tuple), extrap = extrap, deriv = deriv)
end
...
function interp!(
    ...
    method_tuple = method isa AbstractInterpMethod ? ntuple(_ -> method, Val(N)) : method
    _all_linear_methods(method_tuple) || _throw_gridded_nonlinear(method_tuple)
    return linear_interp!(out, grids, data, gq; bc = map(_method_bc, method_tuple), extrap = extrap, deriv = deriv)
end

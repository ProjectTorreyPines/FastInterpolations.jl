...
@inline function _anchor_query(
    ...
    return _linear_anchor_query_impl(x, xq, wrap)
end
...
function _anchor_query(
    ...
    @inbounds for k in eachindex(xq)
        output[k] = _linear_anchor_query_impl(x, T(xq[k]), wrap, policy)
    end
    return output
end
...
@inline function _fill_anchors!(
    ...
    @inbounds for k in eachindex(xq)
        buffer[k] = _linear_anchor_query_impl(x, T(xq[k]), wrap, policy)
    end
    return buffer
end
...
@inline function _linear_eval_with_anchor(
    ...
    return _linear_anchor_dispatch(itp, aq, op, itp.mode)
end

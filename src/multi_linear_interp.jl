...
#
# Include order: ... → linear_anchor.jl → multi_linear_interp.jl
#
...
"""
    LinearMultiInterpolant{T,I}

Container for multiple linear interpolants sharing the same x-grid.
...
# Fields
- `itps::I`: Container of `LinearInterpolant{T}` objects
...
"""
struct LinearMultiInterpolant{T<:AbstractFloat, I<:AbstractArray{<:LinearInterpolant{T}}} <: AbstractMultiInterpolant{T}
    itps::I
end
...
@inline _ref_itp(mitp::LinearMultiInterpolant) = first(mitp.itps)

"""Check if wrap mode is active (for anchor construction)."""
@inline _should_wrap(mitp::LinearMultiInterpolant) = _ref_itp(mitp).mode === Val(:wrap)
...
function linear_interp(
    ...
    itp0 = linear_interp(x, first(ys); extrap=extrap)
    ...
    for (k, y) in Iterators.drop(pairs(ys), 1)
        itps[k] = linear_interp(x, y; extrap=extrap)
    end

    return LinearMultiInterpolant{T, typeof(itps)}(itps)
end
...
function linear_interp(
    ...
    return linear_interp(x, ys; extrap=extrap)
end
...
"""
    (mitp::LinearMultiInterpolant)(xq::Real; deriv=0)

Evaluate multi-Y interpolant at scalar query point (out-of-place).

Returns a vector of values, one per y-series.
"""
function (mitp::LinearMultiInterpolant{T})(xq::S; deriv::Int=0) where {T<:AbstractFloat, S<:Real}
    ref = _ref_itp(mitp)
    xq_typed = T(xq)

    # Build anchor once
    aq = _anchor_query(ref.x, xq_typed, Val(:linear); wrap=_should_wrap(mitp))
...
"""
    (mitp::LinearMultiInterpolant)(output::AbstractVector, xq::Real; deriv=0)

Evaluate multi-Y interpolant at scalar query point (in-place).
"""
function (mitp::LinearMultiInterpolant{T})(
...
"""
    (mitp::LinearMultiInterpolant)(xq::AbstractVector; deriv=0)
...
function (mitp::LinearMultiInterpolant{T})(
    ...
    # Build anchors once
    aq_vec = _anchor_query(ref.x, xq_typed, Val(:linear); wrap=_should_wrap(mitp))
...
"""
    (mitp::LinearMultiInterpolant)(outputs::AbstractVector{<:AbstractVector}, xq::AbstractVector; deriv=0)
...
"""
@with_pool pool function (mitp::LinearMultiInterpolant{T})(
    ...
    _fill_anchors!(aq_vec, ref.x, xq, Val(:linear); wrap=_should_wrap(mitp))
...
"""
    (mitp::LinearMultiInterpolant)(outputs, aq_vec::AbstractVector{<:_LinearAnchoredQuery}; deriv=0)

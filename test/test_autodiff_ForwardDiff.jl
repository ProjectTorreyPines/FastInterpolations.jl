...
@testset "AutoDiff Support" begin
    ...
    @testset "LinearInterpolant with ForwardDiff" begin
        ...
        @testset "derivative matches analytical" begin
            ...
            for xq in test_points
                ...
                @test fd_deriv ≈ analytical atol = 1.0e-10
                @test fd_deriv ≈ 2.0 atol = 1.0e-10  # Known slope
            end
        end
        ...
        @testset "value is preserved" begin
            ...
            for xq in test_points
                ...
                @test fd_value ≈ direct_value atol = 1.0e-10
            end
        end
        ...
        @testset "works with nonlinear data" begin
            ...
            @test fd_deriv ≈ expected_slope atol = 1.0e-10
            @test fd_deriv ≈ analytical atol = 1.0e-10
        end
        ...
        @testset "one-shot API AD" begin
            ...
            @test fd_deriv ≈ analytical atol = 1.0e-10
            @test fd_deriv ≈ 2.0 atol = 1.0e-10  # Known slope for y = 2x + 1
        end
    end
    ...
    @testset "LinearSeriesInterpolant with ForwardDiff" begin
        ...
        @testset "derivative matches analytical (interior points)" begin
            ...
            for xq in test_points
                ...
                @test fd_derivs ≈ analytical atol = 1.0e-10
            end
        end
        ...
        @testset "value is preserved" begin
            ...
            for xq in test_points
                ...
                @test fd_values ≈ sitp(xq) atol = 1.0e-10
            end
        end
        ...
        @testset "linear data exact derivative" begin
            ...
            @test fd_derivs ≈ analytical atol = 1.0e-10
            @test fd_derivs ≈ [2.0, -1.0] atol = 1.0e-10  # d/dx(2x)=2, d/dx(-x+5)=-1
        end
    end
    ...
    @testset "Complex values with ForwardDiff" begin
        ...
        @testset "complex derivative" begin
            ...
            @test fd_deriv ≈ analytical atol = 1.0e-10
            @test real(fd_deriv) ≈ 2.0 atol = 1.0e-10  # d/dx(2x+1)
            @test imag(fd_deriv) ≈ 1.0 atol = 1.0e-10  # d/dx(x-1)
        end
    ...
    @testset "gradient computation" begin
        ...
        @testset "ForwardDiff.gradient on function using interpolant" begin
            ...
            @test grad[1] ≈ expected_grad atol = 1.0e-10
        end
    end
    ...
    @testset "Float32 support" begin
        ...
        @testset "Float32 derivative" begin
            ...
            @test fd_deriv ≈ analytical atol = 1.0e-5
    ...
    @testset "ConstantInterpolant with ForwardDiff" begin
        ...
        @testset "interpolant callable AD - derivative is zero" begin
            ...
            for xq in [0.5, 1.5, 2.5, 3.5]
                fd_deriv = ForwardDiff.derivative(itp, xq)
                analytical = itp(xq; deriv = DerivOp(1))
                @test fd_deriv ≈ 0.0 atol = 1.0e-10
                @test fd_deriv ≈ analytical atol = 1.0e-10
            end
        end
        ...
        @testset "value is preserved" begin
            ...
            for xq in test_points
                ...
                direct_val = itp(xq)
                @test dual_val ≈ direct_val atol = 1.0e-10
            end
        end
    ...
    @testset "Constant complex values with ForwardDiff" begin
        ...
        @testset "complex derivative" begin
            ...
            @test real(fd_deriv) ≈ 0.0 atol = 1.0e-10
            @test imag(fd_deriv) ≈ 0.0 atol = 1.0e-10
            @test fd_deriv ≈ analytical atol = 1.0e-10
        end
    end
    ...
    @testset "QuadraticInterpolant with ForwardDiff" begin
        ...
        @testset "exact quadratic reproduction" begin
            ...
            for xq in test_points
                ...
                expected = 2.0 * xq
                @test fd_deriv ≈ expected atol = 1.0e-10
            end
        end
    ...
    @testset "Quadratic complex values with ForwardDiff" begin
        ...
        @testset "complex derivative" begin
            ...
            @test real(fd_deriv) ≈ 2.0 * 2.25 atol = 1.0e-10
            @test imag(fd_deriv) ≈ 2.0 * 2.25 atol = 1.0e-10
        end
    ...
    @testset "CubicInterpolant with ForwardDiff" begin
        ...
        @testset "Float32 support" begin
            ...
            @test fd_deriv ≈ analytical atol = 1.0e-4
            @test fd_deriv isa Float32
        end
    ...
    @testset "QuadraticSeriesInterpolant with ForwardDiff" begin
        ...
        @testset "quadratic data exact derivative" begin
            ...
            @test fd_derivs ≈ [2.0 * xq, 4.0 * xq] atol = 1.0e-10
        end
        ...
        @testset "Float32 support" begin
            ...
            @test fd_derivs ≈ analytical atol = 1.0e-5
    ...
    @testset "CubicSeriesInterpolant with ForwardDiff" begin
        ...
        @testset "cubic data exact derivative" begin
            ...
            @test fd_derivs ≈ [3.0 * xq^2, 6.0 * xq^2] atol = 1.0e-10
        end
    ...
    @testset "Extrapolation modes with ForwardDiff" begin
        ...
        @testset "Linear extrap modes" begin
            ...
            @testset "extrap=ClampExtrap()" begin
                ...
                @test fd_right ≈ 0.0 atol = 1.0e-10
                @test fd_right ≈ an_right atol = 1.0e-10

                # Outside domain left: derivative is also 0
                fd_left = ForwardDiff.derivative(itp, -1.0)
                an_left = itp(-1.0; deriv = DerivOp(1))
                @test fd_left ≈ 0.0 atol = 1.0e-10
                @test fd_left ≈ an_left atol = 1.0e-10
            end
        ...
        @testset "Cubic extrap modes" begin
            ...
            @testset "extrap=ClampExtrap()" begin
                itp = cubic_interp(x, y_cubic; extrap = ClampExtrap())
        ...
        @testset "Quadratic extrap modes" begin
            ...
            @testset "extrap=ClampExtrap()" begin
                itp = quadratic_interp(x, y_quad; extrap = ClampExtrap())
            ...
            @testset "extrap=WrapExtrap()" begin
                ...
                @test itp(6.5) ≈ itp(1.5) atol = 1.0e-10  # 6.5 mod 5 = 1.5
            end
        end
        ...
        @testset "Value preservation across extrap modes" begin
            ...
            for ext in (ExtendExtrap(), ClampExtrap(), WrapExtrap())
                itp = linear_interp(x, y; extrap = ext)

                # In-domain value test
                xq = 5.5
                dual_val = itp(ForwardDiff.Dual(xq, 1.0))
                @test ForwardDiff.value(dual_val) ≈ itp(xq) atol = 1.0e-10
            end
        end
    end
    ...
    @testset "ConstantInterpolantND with ForwardDiff" begin
        ...
        @testset "partial derivative via heterogeneous tuple" begin
            ...
            df_dx = ForwardDiff.derivative(t -> itp((t, 0.5)), 1.0)
            @test df_dx ≈ 0.0 atol = 1.0e-10

            df_dy = ForwardDiff.derivative(t -> itp((1.0, t)), 0.5)
            @test df_dy ≈ 0.0 atol = 1.0e-10
        end

        @testset "gradient via vector query" begin
            grad = ForwardDiff.gradient(v -> itp(v), [1.0, 0.5])
            @test grad ≈ [0.0, 0.0] atol = 1.0e-10
        end
    ...
    @testset "Value-direction AD (y-AD)" begin
        ...
        @testset "Linear y-AD" begin
            @testset "midpoint gradient" begin
                ...
                @test grad ≈ [0, 0, 0.5, 0.5, 0, 0] atol = 1.0e-12
            end
            ...
            @testset "partition of unity" begin
                for xq in [0.3, 1.7, 2.5, 3.9, 4.1]
                    ...
                    @test sum(grad) ≈ 1.0 atol = 1.0e-12
                end
            end
            ...
            @testset "grid point exactness" begin
                ...
                for k in 2:(length(x) - 1)  # interior grid points
                    ...
                    @test grad[k] ≈ 1.0 atol = 1.0e-12
                    @test sum(abs, grad) ≈ 1.0 atol = 1.0e-12  # no other contributions
                end
            end
        ...
        @testset "Cubic y-AD" begin
            @testset "partition of unity" begin
                for xq in [0.5, 1.5, 2.5, 3.5, 4.5]
                    ...
                    @test sum(grad) ≈ 1.0 atol = 1.0e-10
                end
            end
            ...
            @testset "grid point exactness" begin
                for k in 2:(length(x) - 1)
                    ...
                    @test grad[k] ≈ 1.0 atol = 1.0e-10
                    @test sum(abs, grad) ≈ 1.0 atol = 1.0e-10
                end
            end
        ...
        @testset "value preservation across methods" begin
            ...
            for (name, builder) in [
                ...
                @test builder(y) ≈ builder(y) atol = 1.0e-12
            end
        end
        ...
        @testset "Series y-AD (Jacobian)" begin
            ...
            @testset "Linear series Jacobian" begin
                ...
                @test size(J) == (2, 22)
                @test sum(J[1, :]) ≈ 1.0 atol = 1.0e-10
                @test sum(J[2, :]) ≈ 1.0 atol = 1.0e-10
                # y1 affects only output 1, y2 affects only output 2
                @test sum(abs, J[1, 12:22]) < 1.0e-12  # y2 doesn't affect output 1
                @test sum(abs, J[2, 1:11]) < 1.0e-12   # y1 doesn't affect output 2
            end
            ...
            @testset "Constant series Jacobian" begin
                ...
                @test sum(J[1, :]) ≈ 1.0 atol = 1.0e-10
                @test sum(J[2, :]) ≈ 1.0 atol = 1.0e-10
                @test sum(abs, J[1, 12:22]) < 1.0e-12
                @test sum(abs, J[2, 1:11]) < 1.0e-12
            end

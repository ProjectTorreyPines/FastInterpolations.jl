@testitem "coeffs / CellPoly" begin
    ...
    @testset "cubic coeffs" begin
        ...
        @testset "round-trip at random points" begin
            for xq in [0.05, 0.5, 1.0, 1.73, 2.5, 2.99]
                cell = coeffs(itp, xq)
                @test cell(xq) ≈ itp(xq) atol = 1.0e-12
                @test evalpoly(xq, cell) ≈ itp(xq) atol = 1.0e-12
            end
        end
        ...
        @testset "non-uniform grid" begin
            ...
            for xq in [0.1, 0.5, 1.2, 2.3, 2.9]
                ...
                @test cell(xq) ≈ itp_nu(xq) atol = 1.0e-12
            end
        end
        ...
        @testset "derivative from coefficients" begin
            ...
            for xq in [0.3, 1.5, 2.7]
                ...
                @test deriv_from_coeffs ≈ itp(xq; deriv = DerivOp(1)) atol = 1.0e-10
            end
        end
        ...
        @testset "integration from coefficients" begin
            ...
            for i in 1:5
                ...
                @test integral_from_coeffs ≈ integrate(itp, xL, xR) atol = 1.0e-12
            end
        end
        ...
        @testset "cell boundary continuity" begin
            ...
            for i in 2:(length(x_vec) - 1)
                ...
                @test cell_left(xmid) ≈ cell_right(xmid) atol = 1.0e-10
            end
        end
        ...
        @testset "search keyword" begin
            cell1 = coeffs(itp, 1.5; search = BinarySearch())
            hint = Ref(1)
            cell2 = coeffs(itp, 1.5; search = LinearBinarySearch(linear_window = 0), hint = hint)
    ...
    @testset "quadratic coeffs" begin
        ...
        @testset "round-trip" begin
            for xq in [0.05, 0.5, 1.5, 2.5, 2.99]
                cell = coeffs(itp, xq)
                @test cell(xq) ≈ itp(xq) atol = 1.0e-10
                @test evalpoly(xq, cell) ≈ itp(xq) atol = 1.0e-10
            end
        end
        ...
        @testset "non-uniform grid" begin
            ...
            for xq in [0.1, 0.5, 1.2, 2.3, 2.9]
                cell = coeffs(itp_nu, xq)
                @test cell(xq) ≈ itp_nu(xq) atol = 1.0e-10
            end
        end
    ...
    @testset "complex values" begin
        ...
        @test cell(0.5) ≈ itp(0.5) atol = 1.0e-10
    end
    ...
    @testset "query at grid points" begin
        ...
        for i in 2:(length(x_vec) - 1)
            ...
            @test cell(x_vec[i]) ≈ itp(x_vec[i]) atol = 1.0e-12
        end

        # At first grid point
        cell_first = coeffs(itp, x_vec[1])
        @test cell_first(x_vec[1]) ≈ itp(x_vec[1]) atol = 1.0e-12

        # At last grid point (should resolve to last cell)
        cell_last = coeffs(itp, x_vec[end])
        @test cell_last(x_vec[end]) ≈ itp(x_vec[end]) atol = 1.0e-12
    end

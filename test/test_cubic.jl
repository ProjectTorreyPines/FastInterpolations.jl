...
@testset "Cubic Spline - Core Functionality" begin
    ...
    @testset "Extrapolation :constant" begin
        ...
        result_left = cubic_interp(x, y, -0.5; extrap=ClampedExtrap())
        @test result_left ≈ y[1]

        # Right boundary - returns y[end]
        result_right = cubic_interp(x, y, 1.5; extrap=ClampedExtrap())
        @test result_right ≈ y[end]

        # Vector query
        result = cubic_interp(x, y, [-0.5, 0.5, 1.5]; extrap=ClampedExtrap())
        @test result[1] ≈ y[1]
        @test result[3] ≈ y[end]

        # With cache
        cache = CubicSplineCache(x)
        @test cubic_interp(cache, y, -0.5; extrap=ClampedExtrap()) ≈ y[1]
        @test cubic_interp(cache, y, 1.5; extrap=ClampedExtrap()) ≈ y[end]
...
@testset "Cubic Spline - Uncovered Paths" begin
    ...
    @testset "cubic_interp! with cache and scalar query" begin
        ...
        cubic_interp!(output, cache, y, -0.1; extrap=ClampedExtrap())
        @test output[1] ≈ y[1]

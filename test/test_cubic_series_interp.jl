...
@testset "CubicSeriesInterpolant - Unified Struct Fields" begin
    ...
    @testset "Extrap is AbstractExtrap type" begin
        ...
        mitp_const = cubic_interp(x, Series(y1, y2); extrap=ClampedExtrap())
        @test mitp_const.extrap === ClampedExtrap()
    end
end
...
@testset "CubicSeriesInterpolant - Uncovered Path Coverage" begin
    ...
    @testset "Vector extrapolation ClampedExtrap() mode" begin
        mitp = cubic_interp(x, Series(y1, y2); extrap=ClampedExtrap())
        xq = [-0.1, 0.5, 1.1]  # Include out-of-domain points
    ...
    @testset "Scalar extrapolation ClampedExtrap() with derivative" begin
        mitp = cubic_interp(x, Series(y1, y2); extrap=ClampedExtrap())
    ...
    @testset "Vector extrapolation ClampedExtrap() with derivative" begin
        mitp = cubic_interp(x, Series(y1, y2); extrap=ClampedExtrap())
        xq = [-0.1, 0.5, 1.1]  # Include out-of-domain points
...
@testset "CubicSeriesInterpolant - Constructor Validation" begin
    ...
    @testset "Extrap propagation" begin
        for extrap_mode in (NoExtrap(), ClampedExtrap(), ExtendExtrap(), WrapExtrap())
...
@testset "CubicSeriesInterpolant - Scalar Extrap Modes" begin
    ...
    @testset "extrap=ClampedExtrap() - boundary values" begin
        mitp = cubic_interp(x, Series(y1, y2); extrap=ClampedExtrap())
...
@testset "CubicSeriesInterpolant - Vector Extrap Modes" begin
    ...
    @testset "All extrap modes work with vector" begin
        for mode in (ClampedExtrap(), ExtendExtrap(), WrapExtrap())
...
@testset "CubicSeriesInterpolant - Pre-built Anchor with Extrapolation" begin
    ...
    @testset ":constant mode with pre-built anchors" begin
        mitp = cubic_interp(x, Series(y1, y2, y3); extrap=ClampedExtrap())

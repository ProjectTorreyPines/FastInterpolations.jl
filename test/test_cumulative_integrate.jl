@testitem "cumulative_integrate" begin
    ...
    @testset "1D scalar" begin
        ...
        @testset "constant" begin
            ...
            for side in (LeftSide(), RightSide(), NearestSide())
                ...
                @test cum[1] == 0.0
                @test cum[end] ≈ integrate(itp_k) atol = 1.0e-12
            end
        end
    end
    ...
    @testset "1D analytical" begin
        ...
        for (j, xj) in enumerate(x)
            ...
            @test cum[j] ≈ expected atol = 1.0e-12
        end
    end
    ...
    @testset "1D Series" begin
        ...
        for (name, mk_scalar, mk_series) in [
            ...
            @testset "$name" begin
                ...
                @test cum[:, 1] ≈ cum1 atol = 1.0e-12
                @test cum[:, 2] ≈ cum2 atol = 1.0e-12

                # endpoint matches integrate
                result = integrate(sitp)
                @test cum[end, 1] ≈ result[1] atol = 1.0e-12
                @test cum[end, 2] ≈ result[2] atol = 1.0e-12
            end
        end
        ...
        @testset "constant series" begin
            ...
            for side in (LeftSide(), RightSide(), NearestSide())
                ...
                cum = cumulative_integrate(sitp)
                @test cum[:, 1] ≈ cumulative_integrate(itp1) atol = 1.0e-12
                @test cum[:, 2] ≈ cumulative_integrate(itp2) atol = 1.0e-12
            end
        end
    end

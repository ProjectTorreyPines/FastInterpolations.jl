...
@testset "integrate 1d linear/quadratic/constant" begin
    ...
    @testset "quadratic polynomial exact" begin
        ...
        @test integrate(itp, a, b) ≈ expected atol=1e-10
    end
    ...
    @testset "common invariants" begin
        ...
        for itp in (linear_interp(x, y), quadratic_interp(x, y), constant_interp(x, y))
            ...
            @test integrate(itp, a, b) ≈ -integrate(itp, b, a) atol=1e-12
            @test integrate(itp, a, b) ≈ integrate(itp, a, m) + integrate(itp, m, b) atol=1e-12
        end
    end
end

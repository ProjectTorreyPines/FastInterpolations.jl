...
@testset "Full-domain fast path" begin
    ...
    @testset "1D scalar parity" begin
        ...
        @test integrate(itp_c) ≈ integrate(itp_c, first(x), last(x)) atol = 1.0e-14
        @test integrate(itp_l) ≈ integrate(itp_l, first(x), last(x)) atol = 1.0e-14
        @test integrate(itp_q) ≈ integrate(itp_q, first(x), last(x)) atol = 1.0e-14

        for side in (LeftSide(), RightSide(), NearestSide())
            itp_k = constant_interp(x, y_const; side = side, extrap = NoExtrap())
            @test integrate(itp_k) ≈ integrate(itp_k, first(x), last(x)) atol = 1.0e-14
        end
    end
    ...
    @testset "1D analytical exactness" begin
        ...
        @test integrate(itp_l) ≈ expected_linear atol = 1.0e-12
        ...
        @test integrate(itp_c) ≈ expected_cubic atol = 1.0e-10
    end
    ...
    @testset "1D Series parity" begin
        ...
        for (name, mk_scalar, mk_series) in [
            ...
            @testset "$name" begin
                ...
                @test result[1] ≈ integrate(itp1) atol = 1.0e-12
                @test result[2] ≈ integrate(itp2) atol = 1.0e-12
            end
        end
        ...
        @testset "constant series" begin
            ...
            for side in (LeftSide(), RightSide(), NearestSide())
                sitp = constant_interp(x, Series(y_c[:, 1], y_c[:, 2]); side = side)

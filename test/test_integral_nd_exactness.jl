...
@testset "integrate nd polynomial exactness" begin
    ...
    @testset "2D linear: full-domain bilinear" begin
        ...
        Y1 = hi[2] - lo[2];  Y2 = (hi[2]^2 - lo[2]^2) / 2
        expected = X2*Y2 + 2*X2*Y1 - 3*X1*Y2 + 5*X1*Y1
        @test integrate(itp) ≈ expected atol=1e-12
    end
    ...
    @testset "3D linear: trilinear f = xyz + xy + yz + xz" begin
        ...
        @test integrate(itp, lo, hi) ≈ expected atol=1e-11
    end
    ...
    @testset "3D quadratic: product x²·y²·z²" begin
        ...
        Z3 = (hi[3]^3 - lo[3]^3) / 3
        expected = X3 * Y3 * Z3
        @test integrate(itp, lo, hi) ≈ expected atol=1e-6
    end
    ...
    @testset "3D cubic structural properties" begin
        ...
        @testset "sign rule (antisymmetry)" begin
            ...
            @test integrate(itp, lo, hi) ≈ -integrate(itp, (hi[1], lo[2], lo[3]),
                                                            (lo[1], hi[2], hi[3])) atol=1e-10
            # flip two axes → same sign
            @test integrate(itp, lo, hi) ≈  integrate(itp, (hi[1], hi[2], lo[3]),
                                                            (lo[1], lo[2], hi[3])) atol=1e-10

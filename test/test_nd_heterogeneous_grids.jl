...
@testset "Heterogeneous Grid + Mixed Derivative Regression" begin
    ...
    @testset "2D Cubic: Range × Vector with CubicFit — polynomial analytic" begin
        ...
        # Value
        @test itp((xq, yq); deriv = DerivOp(0, 0)) ≈ f(xq, yq) atol = 1.0e-10

        # First derivatives (mixed derivative orders: one axis differentiated, one not)
        @test itp((xq, yq); deriv = DerivOp(1, 0)) ≈ df_dx(xq, yq)   atol = 1.0e-8
        @test itp((xq, yq); deriv = DerivOp(0, 1)) ≈ df_dy(xq, yq)   atol = 1.0e-6

        # Second derivatives — these are the cases that triggered the original bug
        @test itp((xq, yq); deriv = DerivOp(2, 0)) ≈ d2f_dx2(xq, yq) atol = 1.0e-6
        @test itp((xq, yq); deriv = DerivOp(0, 2)) ≈ d2f_dy2(xq, yq) atol = 1.0e-4
        @test itp((xq, yq); deriv = DerivOp(1, 1)) ≈ d2f_dxdy(xq, yq) atol = 1.0e-6

        # Third derivatives
        @test itp((xq, yq); deriv = DerivOp(3, 0)) ≈ d3f_dx3(xq, yq) atol = 1.0e-4
        @test itp((xq, yq); deriv = DerivOp(0, 3)) ≈ d3f_dy3(xq, yq) atol = 1.0e-2
    end
    ...
    @testset "2D Cubic: Vector × Range with CubicFit — reversed heterogeneous" begin
        ...
        @test itp((xq, yq); deriv = DerivOp(0, 0)) ≈ f(xq, yq)         atol = 1.0e-10
        @test itp((xq, yq); deriv = DerivOp(1, 0)) ≈ df_dx(xq, yq)     atol = 1.0e-6
        @test itp((xq, yq); deriv = DerivOp(0, 1)) ≈ df_dy(xq, yq)     atol = 1.0e-8
        @test itp((xq, yq); deriv = DerivOp(2, 0)) ≈ d2f_dx2(xq, yq)   atol = 1.0e-4
        @test itp((xq, yq); deriv = DerivOp(0, 2)) ≈ d2f_dy2(xq, yq)   atol = 1.0e-6
        @test itp((xq, yq); deriv = DerivOp(1, 1)) ≈ d2f_dxdy(xq, yq)  atol = 1.0e-6
    end
    ...
    @testset "2D Cubic: Vector × Vector (both non-uniform)" begin
        ...
        @test itp((xq, yq); deriv = DerivOp(1, 0)) ≈ df_dx(xq, yq)     atol = 1.0e-6
        @test itp((xq, yq); deriv = DerivOp(0, 1)) ≈ df_dy(xq, yq)     atol = 1.0e-6
        @test itp((xq, yq); deriv = DerivOp(2, 0)) ≈ 2yq                atol = 1.0e-4
        @test itp((xq, yq); deriv = DerivOp(0, 2)) ≈ 2xq                atol = 1.0e-4
        @test itp((xq, yq); deriv = DerivOp(1, 1)) ≈ d2f_dxdy(xq, yq)  atol = 1.0e-4
    end
    ...
    @testset "3D Cubic: Range × Vector × Range with CubicFit — analytic" begin
        ...
        # Value
        @test itp((xq, yq, zq)) ≈ f(xq, yq, zq) atol = 1.0e-10

        # First derivatives — each is a mixed derivative order tuple like (1,0,0)
        @test itp((xq, yq, zq); deriv = DerivOp(1, 0, 0)) ≈ df_dx(xq, yq, zq) atol = 1.0e-6
        @test itp((xq, yq, zq); deriv = DerivOp(0, 1, 0)) ≈ df_dy(xq, yq, zq) atol = 1.0e-6
        @test itp((xq, yq, zq); deriv = DerivOp(0, 0, 1)) ≈ df_dz(xq, yq, zq) atol = 1.0e-6

        # Second derivatives — mixed orders across 3 axes
        @test itp((xq, yq, zq); deriv = DerivOp(2, 0, 0)) ≈ d2f_dx2(xq, yq, zq) atol = 1.0e-4
        @test itp((xq, yq, zq); deriv = DerivOp(0, 2, 0)) ≈ d2f_dy2(xq, yq, zq) atol = 1.0e-4
        @test itp((xq, yq, zq); deriv = DerivOp(0, 0, 2)) ≈ d2f_dz2(xq, yq, zq) atol = 1.0e-4
    end
    ...
    @testset "2D Linear: Range × Vector — value and derivative" begin
        ...
        @test itp((xq, yq)) ≈ f(xq, yq) atol = 1.0e-12
        @test itp((xq, yq); deriv = DerivOp(1, 0)) ≈ 3.0       atol = 1.0e-10
        @test itp((xq, yq); deriv = DerivOp(0, 1)) ≈ 2.0       atol = 1.0e-10
    end
    ...
    @testset "3D Quadratic: Range × Vector × Range — analytic" begin
        ...
        @test itp((xq, yq, zq)) ≈ f(xq, yq, zq) atol = 1.0e-8
        @test itp((xq, yq, zq); deriv = DerivOp(1, 0, 0)) ≈ df_dx(xq, yq, zq) atol = 1.0e-6
        @test itp((xq, yq, zq); deriv = DerivOp(0, 1, 0)) ≈ df_dy(xq, yq, zq) atol = 1.0e-6
        @test itp((xq, yq, zq); deriv = DerivOp(0, 0, 1)) ≈ df_dz(xq, yq, zq) atol = 1.0e-6
        @test itp((xq, yq, zq); deriv = DerivOp(2, 0, 0)) ≈ d2f_dx2(xq, yq, zq) atol = 1.0e-4
    ...
    @testset "2D Quadratic: heterogeneous grid + DerivOp derivative spec" begin
        ...
        @test itp((xq, yq); deriv = DerivOp(1, 0)) ≈ 2xq   atol = 1.0e-8
        @test itp((xq, yq); deriv = DerivOp(0, 1)) ≈ 2yq   atol = 1.0e-6
        @test itp((xq, yq); deriv = DerivOp(2, 0)) ≈ 2.0    atol = 1.0e-4
    end
    ...
    @testset "2D Quadratic: heterogeneous grid + batch evaluation" begin
        ...
        vals = itp((xqs, yqs))
        for k in 1:3
            @test vals[k] ≈ f(xqs[k], yqs[k]) atol = 1.0e-8
        end

        dvals = itp((xqs, yqs); deriv = DerivOp(1, 0))
        for k in 1:3
            @test dvals[k] ≈ 2xqs[k] atol = 1.0e-6
        end
    end
    ...
    @testset "2D Quadratic: heterogeneous grid + mixed search policies" begin
        ...
        xq, yq = 1.0, 0.6
        @test itp((xq, yq)) ≈ f(xq, yq) atol = 1.0e-8
    end
    ...
    @testset "2D Quadratic: heterogeneous grid + mixed extrap modes" begin
        ...
        @test itp((1.0, 0.5)) ≈ f(1.0, 0.5) atol = 1.0e-8
    ...
    @testset "2D Cubic: heterogeneous grid + mixed search policies" begin
        ...
        xq, yq = 1.0, 0.6
        @test itp((xq, yq)) ≈ f(xq, yq)     atol = 1.0e-10
        @test itp((xq, yq); deriv = DerivOp(1, 0)) ≈ 2xq * yq     atol = 1.0e-6
        @test itp((xq, yq); deriv = DerivOp(0, 1)) ≈ xq^2          atol = 1.0e-6
        @test itp((xq, yq); deriv = DerivOp(2, 0)) ≈ 2yq           atol = 1.0e-4
        @test itp((xq, yq); deriv = DerivOp(0, 2)) ≈ 0.0           atol = 1.0e-4
    end
    ...
    @testset "2D Cubic: heterogeneous grid + mixed extrap modes" begin
        ...
        @test itp((1.0, 0.5); deriv = DerivOp(1, 0)) ≈ 0.5 atol = 1.0e-6
        @test itp((1.0, 0.5); deriv = DerivOp(0, 1)) ≈ 1.0 atol = 1.0e-6
    end
    ...
    @testset "2D Cubic: heterogeneous grid + per-axis BC" begin
        ...
        @test itp((xq, yq); deriv = DerivOp(1, 0)) ≈ 2xq * yq^2       rtol = 1.0e-10
        @test itp((xq, yq); deriv = DerivOp(0, 1)) ≈ 2xq^2 * yq       rtol = 1.0e-10
        @test itp((xq, yq); deriv = DerivOp(2, 0)) ≈ 2yq^2            rtol = 1.0e-10
    end
    ...
    @testset "2D Cubic: heterogeneous grid + DerivOp derivative spec" begin
        ...
        @test itp((xq, yq); deriv = DerivOp(0, 1)) ≈ 2yq   atol = 1.0e-6
        @test itp((xq, yq); deriv = DerivOp(2, 0)) ≈ 2.0    atol = 1.0e-4
        @test itp((xq, yq); deriv = DerivOp(0, 2)) ≈ 2.0    atol = 1.0e-4
    end
    ...
    @testset "2D Cubic: heterogeneous grid + batch evaluation" begin
        ...
        for k in 1:3
            @test vals[k] ≈ f(xqs[k], yqs[k]) atol = 1.0e-10
        end

        # Batch derivative
        dvals = itp((xqs, yqs); deriv = DerivOp(1, 0))
        for k in 1:3
            @test dvals[k] ≈ 2xqs[k] * yqs[k] atol = 1.0e-6
        end
    end
end

...
@testset "CubicFit Normalization" begin
    ...
    @testset "_is_periodic_bc for CubicFit" begin
        # CubicFit is not periodic
        @test FastInterpolations._is_periodic_bc(CubicFit()) == false
    end
...
@testset "CubicFit Mathematical Correctness" begin
    ...
    @testset "Smooth Function (sin)" begin
        # sin(x) is smooth; CubicFit should give reasonable results
...
@testset "CubicFit Mixed with Other BCs" begin
    ...
    @testset "Deriv1(exact) Left, CubicFit Right" begin
        # f(x) = x², f'(0) = 0
        f_mixed2(x) = x^2
...
@testset "CubicFit with CubicSplineCache" begin
    ...
    @testset "In-Place API with CubicFit" begin
        # Use Range directly (not collect) - CubicFit requires uniform grid
        x = range(0.0, 1.0, 21)
        y = sin.(π .* collect(x))
        cache = CubicSplineCache(x; bc=CubicFit())

...
@testset "Quadratic Interpolation - Public API" begin
    ...
    @testset "quadratic_interp extrapolation" begin
        ...
        @test quadratic_interp(x, y, -0.5; extrap=ClampedExtrap()) ≈ 0.0
        @test quadratic_interp(x, y, 2.5; extrap=ClampedExtrap()) ≈ 4.0

        # :constant - inside domain should work normally (coverage for eval_core path)
        @test quadratic_interp(x, y, 1.0; extrap=ClampedExtrap()) ≈ 1.0

        # :constant - derivatives return zero outside domain
        @test quadratic_interp(x, y, -0.5; extrap=ClampedExtrap(), deriv=DerivOp(1)) ≈ 0.0
        @test quadratic_interp(x, y, 2.5; extrap=ClampedExtrap(), deriv=DerivOp(1)) ≈ 0.0
        @test quadratic_interp(x, y, -0.5; extrap=ClampedExtrap(), deriv=DerivOp(2)) ≈ 0.0
        @test quadratic_interp(x, y, 2.5; extrap=ClampedExtrap(), deriv=DerivOp(2)) ≈ 0.0
...
@testset "Quadratic Interpolation - MinCurvFit API" begin
    ...
    @testset "MinCurvFit extrapolation modes" begin
        ...
        # :constant mode
        itp_const = quadratic_interp(x, y; bc=MinCurvFit(), extrap=ClampedExtrap())

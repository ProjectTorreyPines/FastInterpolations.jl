...
@testitem "Stateful payload: only construction sites may name the oneunit param" begin
    ...
    for (root, _, files) in walkdir(src_dir), f in files
        ...
        for (i, line) in enumerate(lines)
            occursin(naming_U, line) || continue
            startswith(lstrip(line), "#") && continue
            # The call form may sit a few lines below the signature that names it.
    ...
    if !isempty(offenders)
        @info "dispatch-only arms naming the oneunit param (use `<:_StatefulPayload{P}`)" offenders
    end

...
@testitem "Thread Safety" begin
    ...
    @testset "Workspace Correctness" begin
        @testset "Scalar query" begin
            ...
            @test count(e -> e > 1.0e-10, errors) == 0
        end
        ...
        @testset "Lazy transpose concurrent initialization" begin
            ...
            @threads for i in 1:n_iter
                try
                    ...
                    @assert result[1] ≈ itp1(xq) atol = 1.0e-12
                    @assert result[2] ≈ itp2(xq) atol = 1.0e-12
                catch
                    atomic_add!(errors, 1)
                end
            end
        ...
        @testset "Concurrent scalar evaluation" begin
            ...
            @threads for i in 1:n_iter
                try
                    ...
                    @assert result[1] ≈ itp1(xq) atol = 1.0e-12
                    @assert result[2] ≈ itp2(xq) atol = 1.0e-12
                    @assert result[3] ≈ itp3(xq) atol = 1.0e-12
                catch
                    atomic_add!(errors, 1)
                end
            end
        ...
        @testset "Concurrent in-place scalar evaluation" begin
            ...
            @threads for i in 1:n_iter
                try
                    ...
                    # Verify correctness against single-interpolant results
                    @assert output[1] ≈ itp1(xq) atol = 1.0e-12
                    @assert output[2] ≈ itp2(xq) atol = 1.0e-12
                catch
                    atomic_add!(errors, 1)
                end
            end
        ...
        @testset "Concurrent vector evaluation" begin
            ...
            @threads for i in 1:n_iter
                try
                    ...
                    @assert outputs[1] ≈ expected1 atol = 1.0e-12
                    @assert outputs[2] ≈ expected2 atol = 1.0e-12
                catch
                    atomic_add!(errors, 1)
                end
            end
    ...
    @testset "Search Policy Thread Safety" begin
        @testset "LinearBinarySearch concurrent access" begin
            ...
            @threads for i in eachindex(results)
                try
                    results[i] = itp(rand(); search = LinearBinarySearch())
                catch
                    atomic_add!(errors, 1)
                end
            end
        ...
        @testset "LinearBinarySearch{0} concurrent access" begin
            x = collect(range(0.0, 1.0, 1001))
            y = sin.(2π .* x)
            itp = linear_interp(x, y)
            ...
            @threads for i in eachindex(results)
                try
                    results[i] = itp(rand(); search = LinearBinarySearch(linear_window = 0))
                catch
                    atomic_add!(errors, 1)
                end
            end
        ...
        @testset "BinarySearch concurrent access" begin
            x = collect(range(0.0, 1.0, 1001))
            y = sin.(2π .* x)
            itp = linear_interp(x, y)
            ...
            @threads for i in eachindex(results)
                try
                    results[i] = itp(rand(); search = BinarySearch())
                catch
                    atomic_add!(errors, 1)
                end
            end
